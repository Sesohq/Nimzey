...
// Simple function to generate direct node previews without complex processing chains
  ...
  try {
    ...
    if (node.type === 'filterNode') {
      ...
      switch(data.filterType) {
        ...
        // Adding more basic CSS filters (these might not match FilterType exactly)
        case 'findEdges':
        case 'sharpen':
        case 'distort':
        case 'noise':
        ...
        case 'dither':
        case 'saturation':
          // For any complex filter, we'll just add a label
      ...
      // For blend nodes, just add a label
      ctx.fillStyle = 'rgba(0,0,0,0.5)';
      ctx.fillRect(0, 0, canvas.width, 20);
      ctx.fillStyle = '#fff';
      ctx.font = '12px sans-serif';
      ctx.fillText('Blend Node', 5, 15);
    }
